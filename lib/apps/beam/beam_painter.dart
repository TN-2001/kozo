      ...
      for(int i = 0; i < nodes.length; i++){
        ...
        if(nodes[i].loadXY[1] != 0){ // 集中荷重
          if(nodes[i].loadXY[1] < 0){
            MyPainter.arrow(Offset(pos.dx, pos.dy-75), Offset(pos.dx, pos.dy-5), 5, const Color.fromARGB(255, 0, 63, 95), canvas);
          }else{
            MyPainter.arrow(Offset(pos.dx, pos.dy+75), Offset(pos.dx, pos.dy+5), 5, const Color.fromARGB(255, 0, 63, 95), canvas);
          }
        }
      ...
      for(int i = 0; i < elems.length; i++){
        if(elems[i].nodeList[0] != null && elems[i].nodeList[1] != null && elems[i].load != 0.0){
          ...
          for(int j = 0; j <= count; j++){
            ...
            if(elems[i].load < 0){
              MyPainter.arrow(Offset(cpos.dx, cpos.dy-50), Offset(cpos.dx, cpos.dy-5), 3, const Color.fromARGB(255, 0, 63, 95), canvas);
            }
            else{
              MyPainter.arrow(Offset(cpos.dx, cpos.dy+50), Offset(cpos.dx, cpos.dy+5), 3, const Color.fromARGB(255, 0, 63, 95), canvas);
            }
